...
import com.qq.crawler.ReviewInfo;
...
public class ReviewInfoDao extends Dao{
	...
	public int insertListenInfo(ReviewInfo rInfo){
		int status = -1;
		String insertData = "insert into qxreview(id, mid, reviewer, location, content, time)values(?,?,?,?,?,?)";
		try {
			PreparedStatement pstmt = (PreparedStatement) conn.prepareStatement(insertData);
			conn.setAutoCommit(false);
			pstmt.setString(1, rInfo.id);
			pstmt.setString(2, rInfo.mid);
			...
			e.printStackTrace();
			System.err.println("Cannot insert info into table \"review\". ");
		}
		return status;
	}
	public ArrayList<Map<String,String>> getResult(String cols,String tab){
		String queryString = "select " + cols+ " from " + tab + " where id not in (select id from qxreview)";
